...
namespace poekmoncsharp
{
    public class ToGPU
    {
        long ARRAY_SIZE = 8;
        public unsafe int Send(long ammount, long seedOffset, bool showIndividualResults)
        {
            Console.WriteLine("Preparing to launch GPU kernel");
            ARRAY_SIZE = ammount;
            var cl = CL.GetApi();
            nint context = 0;
            nint commandQueue = 0;
            ...
            program = CreateProgram(cl, context, device, "Kernel.cl");
            ...
            long[] a = new long[1];
            a[0] = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
            if (!CreateMemObjects(cl, context, memObjects, a))
            {
                Cleanup(cl, context, commandQueue, program, kernel, memObjects);
                return 0;
            }
            ...
            nuint[] globalWorkSize = new nuint[1] { (nuint)ARRAY_SIZE };
            nuint[] localWorkSize = new nuint[1] { 1 };

            // Queue the kernel up for execution across the array
            Console.WriteLine("queueing kernel...");
            ...
            fixed (void* pValue = result)
            {
                ...
                errNum = cl.EnqueueReadBuffer(commandQueue, memObjects[1], true, 0, (nuint)(ARRAY_SIZE * sizeof(int)), pValue, 0, null, null);
                if (errNum != (int)ErrorCodes.Success)
                {
            ...
            Console.WriteLine("GPU side of the program ended succesfully.");
            Cleanup(cl, context, commandQueue, program, kernel, memObjects);
            return result.Max();
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        unsafe bool CreateMemObjects(CL cl, nint context, nint[] memObjects, long[] a)
        {
            fixed (void* pa = a)
            {
                memObjects[0] = cl.CreateBuffer(context, MemFlags.ReadOnly | MemFlags.CopyHostPtr, (nuint)(sizeof(long) * 1), pa, null);
            }

            memObjects[1] = cl.CreateBuffer(context, MemFlags.ReadWrite, (nuint)(sizeof(int) * ARRAY_SIZE), null, null);

            if (memObjects[0] == IntPtr.Zero || memObjects[1] == IntPtr.Zero)
            {
